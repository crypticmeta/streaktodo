...
- [x] Add analytics — Mixpanel via [`mixpanel-react-native`](https://github.com/mixpanel/mixpanel-react-native), wrapper at [`src/lib/analytics.ts`](./src/lib/analytics.ts). Lazy init, always-on by default (mute locally with `EXPO_PUBLIC_MIXPANEL_DISABLE=1`), best-effort error swallow. Token in `branding.js` (`mixpanelToken`, override via `EXPO_PUBLIC_MIXPANEL_TOKEN`). User profile pushed via `identify` on sign-in / silent restore (Google ID + email + name + avatar + platform). V0 event set: `app_opened`, `onboarding_completed`, `task_created` / `task_completed` / `task_uncompleted` / `task_deleted`, `category_created` / `category_deleted`, `notification_scheduled`, `theme_changed`. Native rebuild required after install (`npx expo run:android`).
